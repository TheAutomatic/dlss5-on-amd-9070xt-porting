...
With this flag, `EnqueueHip` returns `LMXXF_NR_OK` after a verified zero clear of its private neural output; the decoder can then use the original Color. `GetLastError` contains a recovery diagnostic for that call. The host must submit the consumer before `Retire`, `Drain`, or `Destroy`. The Runtime drains a changed consumer queue before it reuses or frees the output. If clearing fails or completion is uncertain, `EnqueueHip` returns `LMXXF_NR_FAILED`; do not submit that consumer or reuse the session.
