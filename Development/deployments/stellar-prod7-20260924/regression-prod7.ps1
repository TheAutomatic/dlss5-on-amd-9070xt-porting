...
$ErrorActionPreference='Stop';$r='D:\DLSSNR-Lab\hip-backend';$d="$r\network-fixed-shapes\regression-prod7";$a='D:\DLSSNR-Lab\Magpie-DLSS5-AMD-0.23\DLSS5-AMD'
...
function One($tag,$height,$seq,$candidate,$frames,$temporal=0,$extra=@()){
 ...
 $m=if($candidate){"$r\network-fixed-shapes\prod7-modules"}else{"$r\network-fixed-shapes\prod2-modules"}
 $runner=if($candidate){'benchmark_main_reuse.exe'}else{'benchmark_main_reuse.exe'}
